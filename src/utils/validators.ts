...
export class EnvironmentValidator {
  ...
  public validate(requiredEnvVar = this.requiredVariables): void {
    const missingVariables = requiredEnvVar.filter(
    ...
    if (missingVariables.length > 0) {
      ...
      throw new Error(
        "Internal Server Error: Missing required environment variables"
      );
    }
  }
}
...
export const validateVerifyRequestBody = (
  event: APIGatewayProxyEvent
): ValidationResult<{ email: any; code: any }> => {
  const body = parseBody(event);
